...
module.exports = function (action, $) {
    ...
    var path  = require("path"),
        restV1 = require(path.join(__dirname, "function"));
    ...
    action.on("actions/*/:deleteAllCookies"                         , restV1.deleteAllCookies      , "action", "deleteAllCookies"   , "Delete all the cookies stored in the browser", true);
    action.on("actions/*/:field:parseJSON:storeAs"                  , restV1.parseJSON            , "action", "parseJSON"   , "Parse JSON object with a field in it and store it", true);
    action.on("actions/*/:storeAs:stringifyJSON"                    , restV1.stringifyJSON        , "action", "stringifyJSON"   , "Stringify JSON object and store it as a string", true);
    action.on("actions/*/:getip"                                    , restV1.getip                , "action", "getip"   , "Gets and stores the ip", true);
    action.on("actions/*/:getip:storeIn"                            , restV1.getipStoreIn         , "action", "getipStoreIn"   , "Gets and stores the ip in a variable", true);
    action.on("actions/*/:body:headers:post:storeResponse"          , restV1.post                 , "action", "post"    , "POST to a REST url and store the result", true);
    action.on("actions/*/:body:get:headers:storeResponse"           , restV1.get                  , "action", "get"      , "GET to a REST url and store the result", true);
    action.on("actions/*/:body:delete:headers:storeResponse"        , restV1.del                  , "action", "delete"      , "DELETE to a REST url and store the result", true);
    action.on("actions/*/:assert:error:is"                          , restV1.assertError          , "action", "assert"      , "Assert that an error value matches", true);
