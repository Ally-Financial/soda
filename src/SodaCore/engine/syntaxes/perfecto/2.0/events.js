...
module.exports = function (action, $) {
    ...
    var path     = require("path"),
        perfectoV2 = require(path.join(__dirname, "function"));
    ...
    action.on("actions/*/:options:swipe:refresh:using"              , perfectoV2.swipeUsingOptions  , "action", "swipe", "Swipe an element, with the provided options. If the value of refresh is false, the DOM will not be reloaded");
    action.on("actions/*/:options:swipe:using"                      , perfectoV2.swipeUsingOptions  , "action", "swipe", "Swipe an element, with the provided options, using a name, label, id, value, or selector"                   );
    action.on("actions/*/:swipe:using"                              , perfectoV2.swipeUsing         , "action", "swipe", "Swipe an element, using a name, label, id, value, or selector"                                              );
    action.on("actions/*/:options:refresh:swipe"                    , perfectoV2.swipeOptions       , "action", "swipe", "Swipe an element, with the provided options. If the value of refresh is false, the DOM will not be reloaded");
    action.on("actions/*/:options:swipe"                            , perfectoV2.swipeOptions       , "action", "swipe", "Swipe an element, with the provided options"                                                                );
    action.on("actions/*/:refresh:swipe"                            , perfectoV2.swipe              , "action", "swipe", "Swipe an element, If the value of refresh is false, the DOM will not be reloaded"                           );
    action.on("actions/*/:swipe"                                    , perfectoV2.swipe              , "action", "swipe", "Swipe an element, using the default options"                                                                );

    action.on("actions/*/:hideAppFor"                               , perfectoV2.hideAppFor         , "action", "hideAppFor"   , "Hide the application for the duration specified", true);

    action.on("actions/*/:monkey"                                   , perfectoV2.monkey             , "action", "monkey"       , "Tap n random elements", true);
    action.on("actions/*/:monkey:selector"                          , perfectoV2.monkey             , "action", "monkey"       , "Tap n random elements, filtering with the given selector", true);
    action.on("actions/*/:monkey:randomTouchEffects"                , perfectoV2.monkey             , "action", "monkey"       , "Tap n random elements and using a random number of touches and a random touch duration", true);
    action.on("actions/*/:monkey:randomTouchEffects:selector"       , perfectoV2.monkey             , "action", "monkey"       , "Tap n random elements, filtering with the given selector and using a random number of touches and a random touch duration", true);

    action.on("actions/*/:monkey:randomRotation:"                            , perfectoV2.monkey    , "action", "monkey"       , "Tap n random elements, with occasional random device rotation", true);
    action.on("actions/*/:monkey:randomRotation:selector"                    , perfectoV2.monkey    , "action", "monkey"       , "Tap n random elements, filtering with the given selector and occasional random device rotation", true);
    action.on("actions/*/:monkey:randomRotation:randomTouchEffects"          , perfectoV2.monkey    , "action", "monkey"       , "Tap n random elements and using a random number of touches, a random touch duration, and occasional random device rotation", true);
    action.on("actions/*/:monkey:randomRotation:randomTouchEffects:selector" , perfectoV2.monkey    , "action", "monkey"       , "Tap n random elements, filtering with the given selector, using a random number of touches and a random touch duration, and occasional random device rotation", true);

    action.on("actions/*/:superMonkey"                              , perfectoV2.superMonkey        , "action", "superMonkey"  , "Tap n random screen points", true);
    action.on("actions/*/:ultraMonkey"                              , perfectoV2.ultraMonkey        , "action", "ultraMonkey"  , "Tap n random screen points", true);

    action.on("actions/*/:options:using:refresh:tap"                , perfectoV2.tapUsingOptions    , "action", "tap", "Tap an element, using the provided options"                                                   );
    action.on("actions/*/:options:tap:using"                        , perfectoV2.tapUsingOptions    , "action", "tap", "Tap an element, using the provided options and using a name, label, id, value, or selector"   );
    action.on("actions/*/:tap:using:refresh"                        , perfectoV2.tapUsing           , "action", "tap", "Tap an element. If the value of refresh is false, the DOM will not be reloaded"               );
    action.on("actions/*/:tap:using"                                , perfectoV2.tapUsing           , "action", "tap", "Tap an element, using a name, label, id, value, or selector"                                  );
    action.on("actions/*/:options:tap"                              , perfectoV2.tapOptions         , "action", "tap", "Tap an element, using the provided options"                                                   );
    action.on("actions/*/:refresh:tap"                              , perfectoV2.tap                , "action", "tap", "Tap an element, setting refresh to false will prevent the DOM from reloading"                 );
    action.on("actions/*/:tap"                                      , perfectoV2.tap                , "action", "tap", "Tap an element, using the default options"                                                    );
    action.on("actions/*/:duration:tap"                             , perfectoV2.longPress          , "action", "tap", "Tap an element, using a duration for long press"                                                    );

    action.on("actions/*/:tapAll:options:refresh:using"              , perfectoV2.tapAllUsingOptions  , "action", "tapAll", "Tap all elements in the returned set, one after another, using the provided options object and using a name, id, label, value, or selector. If the value of refresh evaluates to false, the DOM will not be refreshed");
    action.on("actions/*/:tapAll:options:using"                      , perfectoV2.tapAllUsingOptions  , "action", "tapAll", "Tap all elements in the returned set, one after another, using the provided options object and using a name, id, label, value, or selector"                                                                           );
    action.on("actions/*/:tapAll:using"                              , perfectoV2.tapAllUsing         , "action", "tapAll", "Tap all elements in the returned set, one after another, using a name, id, label, value, or selector"                                                                                                                 );
    action.on("actions/*/:tapAll:options"                            , perfectoV2.tapAllOptions       , "action", "tapAll", "Tap all elements in the returned set, one after another, using the provided options object"                                                                                                                           );
    action.on("actions/*/:tapAll"                                    , perfectoV2.tapAll              , "action", "tapAll", "Tap all elements in the returned set, one after another"                                                                                                                                                              );

    action.on("actions/*/:tapIfExists"                              , perfectoV2.tapIfExists         , "action", "tapIfExists", "Tap an element using the default options, if it exists.");

    action.on("actions/*/:tapXY"                                    , perfectoV2.tapXY              , "action", "tapXY", "Tap the specifed screen coordinates. Expects an array (e.g. [x, y])"                                                                );
    action.on("actions/*/:refresh:tapXY"                            , perfectoV2.tapXY              , "action", "tapXY", "Tap the specifed screen coordinates. Expects an array (e.g. [x, y]). If the value of refresh is false, the DOM will not be reloaded");

    action.on("actions/*/:relativeTo:tapXY"                         , perfectoV2.tapXYRelativeTo    , "action", "tapXY", "Tap the specifed screen coordinates, relative to the specified element's position. Expects an array (e.g. [x, y])"                                                                );
    action.on("actions/*/:refresh:relativeTo:tapXY"                 , perfectoV2.tapXYRelativeTo    , "action", "tapXY", "Tap the specifed screen coordinates, relative to the specified element's position. Expects an array (e.g. [x, y]). If the value of refresh is false, the DOM will not be reloaded");

    action.on("actions/*/:typeOnKeyboard"                           , perfectoV2.typeOnKeyboard     , "action", "typeOnKeyboard" , "Type on the device's keyboard"                , true);
    action.on("actions/*/:resetAppData"                             , perfectoV2.resetAppData       , "action", "resetAppData"   , "Reset the simulator/emulator application data", true);
    action.on("actions/*/:back"                                     , perfectoV2.back               , "action", "back"           , "Click the hardware back button (Android and Perfecto only)", true);
    ...
    action.on("actions/*/:scrollToVisible"                           , perfectoV2.scrollToVisible     , "action", "scrollToVisible", "Scroll an element into view");
    action.on("actions/*/:scrollToVisibleText"                       , perfectoV2.scrollToVisibleText , "action", "scrollToVisibleText", "Scroll an element by text into view");
    action.on("actions/*/:maxAttempts:scrollToVisible"               , perfectoV2.scrollToVisible     , "action", "scrollToVisible", "Scroll an element into view, setting the maxiumum number of scroll attempts (Android Only)"                                );
    action.on("actions/*/:parent:scrollToVisible"                    , perfectoV2.scrollToVisible     , "action", "scrollToVisible", "Scroll an element into view, setting the maxiumum number of scroll attempts (Android Only)"                                );
    action.on("actions/*/:amount:maxAttempts:parent:scrollToVisible" , perfectoV2.scrollToVisible     , "action", "scrollToVisible", "Scroll an element into view, setting the maxiumum number of scroll attempts and the specified scroll amount (Android Only)");
    action.on("actions/*/:amount:maxAttempts:scrollToVisible"        , perfectoV2.scrollToVisible     , "action", "scrollToVisible", "Scroll an element into view, setting the maxiumum number of scroll attempts and the specified scroll amount (Android Only)");
    action.on("actions/*/:amount:scrollToVisible"                    , perfectoV2.scrollToVisible     , "action", "scrollToVisible", "Scroll an element into view, using the specified scroll amount (Android Only)"                                             );
    action.on("actions/*/:amount:parent:scrollToVisible"             , perfectoV2.scrollToVisible     , "action", "scrollToVisible", "Scroll an element into view, using the specified parent and the specified scroll amount (Android Only)"                    );
    action.on("actions/*/:maxAttempts:parent:scrollToVisible"        , perfectoV2.scrollToVisible     , "action", "scrollToVisible", "Scroll an element into view, using the specified parent, setting the maxiumum number of scroll attempts (Android Only)"    );
    ...
    action.on("actions/*/:scrollToTop:type:validate"                , perfectoV2.scrollToTop          , "action", "scrollToTop", "Scroll an element up all the way and validate a file of type 'type' on each scroll (Android Only)");
    action.on("actions/*/:scrollToTop:validate"                     , perfectoV2.scrollToTop          , "action", "scrollToTop", "Scroll an element up all the way and validate a screen on each scroll (Android Only)");
    action.on("actions/*/:execute:scrollToTop"                      , perfectoV2.scrollToTop          , "action", "scrollToTop", "Scroll an element up all the way and execute a file on each scroll (Android Only)");
    action.on("actions/*/:scrollToTop"                              , perfectoV2.scrollToTop          , "action", "scrollToTop", "Scroll an element up all the way (Android Only)");

    action.on("actions/*/:scrollToBottom:type:validate"             , perfectoV2.scrollToBottom       , "action", "scrollToBottom", "Scroll an element down all the way and validate a file of type 'type' on each scroll (Android Only)");
    action.on("actions/*/:scrollToBottom:validate"                  , perfectoV2.scrollToBottom       , "action", "scrollToBottom", "Scroll an element down all the way and validate a screen on each scroll (Android Only)");
    action.on("actions/*/:execute:scrollToBottom"                   , perfectoV2.scrollToBottom       , "action", "scrollToBottom", "Scroll an element down all the way and execute a file on each scroll (Android Only)");
    action.on("actions/*/:scrollToBottom"                           , perfectoV2.scrollToBottom       , "action", "scrollToBottom", "Scroll an element down all the way (Android Only)");
    ...
    action.on("actions/*/:args:stopApp"                              , perfectoV2.stopApp             , "action", "stop",    "Stop an application"                                             , true);
    action.on("actions/*/:openApp"                                   , perfectoV2.openApp            , "action", "open",    "Open an application"                                             , true);
    action.on("actions/*/:closeApp"                                  , perfectoV2.closeApp            , "action", "close",    "Closes an application"                                             , true);
    action.on("actions/*/:homeScreen"                                , perfectoV2.homeScreen          , "action", "homeScreen",    "To home screen"                                             , true);
    action.on("actions/*/:deviceSwipe"                               , perfectoV2.deviceSwipe          , "action", "deviceSwipe", "Swipe on the device"                                                                , true);
    action.on("actions/*/:deviceSwipeLeft"                           , perfectoV2.deviceSwipeLeft     , "action", "deviceSwipe", "Swipe left on the device"                                                                , true);
    action.on("actions/*/:deviceSwipeRight"                          , perfectoV2.deviceSwipeRight    , "action", "deviceSwipe", "Swipe right on the device"                                                                , true);
    action.on("actions/*/:deviceSwipeUp"                             , perfectoV2.deviceSwipeUp     , "action", "deviceSwipe", "Swipe up on the device"                                                                , true);
    action.on("actions/*/:deviceSwipeDown"                           , perfectoV2.deviceSwipeDown    , "action", "deviceSwipe", "Swipe down on the device"                                                              , true);
    action.on("actions/*/:persona"                                   , perfectoV2.persona            , "action", "persona", "Set the persona, values can be <br><br><span class=\"bold\">Georgia, Peter, Ross, Sam, Sara</span>"   , true);
    action.on("actions/*/:lockScreen"                                , perfectoV2.lockScreen         , "action", "lockScreen", "Locks the scren for number of seconds indicated"   , true);
    action.on("actions/*/:sendKeyCommand"                            , perfectoV2.sendKeyCommand     , "action", "sendKeyCommand",    "Sends a key command to the device"                                             , true);
    ...
    action.on("actions/*/:rotateDevice"                    , perfectoV2.rotateDevice                , "action", "rotateDevice", "Rotate the device"                                             , true);
