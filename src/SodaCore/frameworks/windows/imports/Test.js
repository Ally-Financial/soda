 ...
 * @constructor
 */
var WindowsDriver = function () {
    var GET_AVAILABLE_APPLICATIONS = '[{\"command\": \"getAvailableApplications\"}]',
    ...
    function sendWindowsCommand(done, command, count) {
        ...
        client.connect(11000, process.env.WINDOWS_SERVER, function () {
     ...
     * @param {Function} done A callback for completion
     * @param {string} processName The name of the process to get the screenshot from
     */
    ...
    this.getSize = function(done) {
        sendWindowsCommand(done, '[{\"command\": \"getsize\"}');
    };
};
