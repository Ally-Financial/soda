...
/**
 * @module Soda/AssetDrivers/GherkinFiles
 */
 ...
 * @function
 * @memberof module.Soda/AssetDrivers/GherkinFiles
    ...
    if(soda.config.get("ignoreTestDirectories").indexOf(nodePath.basename(directory)) === -1) {
        fs.readdir(directory, function (err, list) {
            ...
            switch (true) { // Set some context based on the directory name
                ...
                case basename === "features":
                    context.type = "feature";
                    break;

                case basename === "givens":
                    context.type = "given";
                    break;

                case basename === "scenarios":
                    context.type = "scenario";
                    break;
 ...
 * @extends BaseDriver
 */
var GherkinFiles = function (soda, collection, path) {
     ...
     * @param  {Function} done A callback for completion
     * @return {GherkinFiles} The current GherkinFiles instance
     */
    ...
    /**
     * An alias for GherkinFiles.load
     * @type {Function}
     */
        ...
        if(typeof asset          === "object" &&
           ...
           (typeof asset.contents === "object" || typeof asset.contents === "string")
            ...
            if(typeof asset.contents !== "string") {
                try {
                    data = JSON.stringify(asset.contents, null, '    ');
                }
                catch (e) {
            ...
            fs.mkdir(dir, { recursive: true }, function (err) {
                ...
                fs.writeFile(path, asset.contents, function (err) {
                    if(done instanceof Function) done.call(self, err || null);
                });
            });
        }
        else {
            if(done instanceof Function) done.call(self, new Error("Invalid arguments passed to GherkinFiles.makeAsset"));
        }
        return self;
    };
        ...
        if(typeof suite === "string") {
            fsPromises.mkdir(nodePath.join(soda.config.get("testPath"), suite.withoutTrailingSlash), { recursive: true }).then( made => {
                ...
                fs.writeFile(nodePath.join(soda.config.get("testPath"), suite.withoutTrailingSlash, "suite.json"), JSON.stringify(suitejson, null, '    '), function (err) {
                    ...
                    fs.mkdir(nodePath.join(soda.config.get("testPath"), suite, "common", "features"  ), { recursive: true}, onMake);
                    fs.mkdir(nodePath.join(soda.config.get("testPath"), suite, "common", "givens"), { recursive: true}, onMake);
                    fs.mkdir(nodePath.join(soda.config.get("testPath"), suite, "common", "scenarios"), { recursive: true}, onMake);
                    fs.mkdir(nodePath.join(soda.config.get("testPath"), suite, "common", "thens"  ), { recursive: true}, onMake);
                    fs.mkdir(nodePath.join(soda.config.get("testPath"), suite, "common", "whens" ), { recursive: true}, onMake);
                });
            });
        }
        else {
            if(done instanceof Function) done.call(self, new Error("Invalid arguments passed to GherkinFiles.makeSuite"));
        }
    };
    ...
    this.makeModule = function (suite, module, done) {
        fsPromises.mkdir(nodePath.join(soda.config.get("testPath"), suite, "modules", module), { recursive: true }).then( made => {
            ...
            fs.writeFile(nodePath.join(soda.config.get("testPath"), suite, "modules", module, "module.json"), JSON.stringify(modulejson, null, '    '), function (err) {
                ...
                // Add test folders
                fs.mkdir(nodePath.join(soda.config.get("testPath"), suite, "modules", module, "features"  ), { recursive: true }, onMake);
                fs.mkdir(nodePath.join(soda.config.get("testPath"), suite, "modules", module, "givens"), { recursive: true }, onMake);
                fs.mkdir(nodePath.join(soda.config.get("testPath"), suite, "modules", module, "scenarios"), { recursive: true }, onMake);
                fs.mkdir(nodePath.join(soda.config.get("testPath"), suite, "modules", module, "thens"  ), { recursive: true }, onMake);
                fs.mkdir(nodePath.join(soda.config.get("testPath"), suite, "modules", module, "whens" ), { recursive: true }, onMake);
            });
        });
        return self;
    };
...
util.inherits(GherkinFiles, EventEmitter);
module.exports = GherkinFiles;
